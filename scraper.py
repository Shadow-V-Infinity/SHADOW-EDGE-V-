...
def get_events(sport_key, slot=None):
    ...
    try:
        ...
        filtered = [
            ...
            and e["status"].get("type", {}).get("name", "") == "notStarted"
            and today_start <= (e.get("startTimestamp") or 0) < today_end
        ]

        if slot and slot in SLOTS:
            filtered = [e for e in filtered if match_in_slot(e.get("startTimestamp"), slot)]
            print(f"  ✅ {len(filtered)} matchs [{slot}] (sur {len(events)} total)")
        else:
...
def get_odds(event_id, home=None, away=None):
    ...
    if home and away and FOOTBALL_DATA_KEY:
        try:
            ...
            fd_matches = _get_fd_cache()
            key = f"{home}_{away}"
            fd  = fd_matches.get(key) or next(
                (v for k, v in fd_matches.items()
                 if home[:6].lower() in k.lower() and away[:6].lower() in k.lower()), None
            )
            if fd:
                print(f"    💡 Cotes Football Data ({home} vs {away})")
                return fd
        except:
            pass
    return {}
...
send_to_railway = send_to_server
