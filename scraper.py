...
# ── ODDS-API.IO — cache par sport ───────────────────────
_odds_cache      = {}  # {sport: {home_away_key: {1, X, 2}}}
_valuebets_cache = {}  # {sport: [{match, value, odd, ...}]}
...
def fetch_odds_for_sport(sport):
    """Charge toutes les cotes du jour pour un sport via odds-api.io."""
    if not ODDS_API_KEY or sport in _odds_cache:
        return _odds_cache.get(sport, {})
    sport_key = ODDS_SPORT_MAP.get(sport, sport)
    result = {}
    try:
        r = requests.get(
            f"{ODDS_API_BASE}/events",
            params={"apiKey": ODDS_API_KEY, "sport": sport_key, "limit": 200},
            timeout=10
        )
        if r.status_code != 200:
            print(f"  ⚠️  Odds-API events {sport}: {r.status_code}")
            return {}
        events = r.json().get("data", [])
        for ev in events:
            home = ev.get("homeTeam", {}).get("name", "") or ev.get("home", "")
            away = ev.get("awayTeam", {}).get("name", "") or ev.get("away", "")
            eid  = ev.get("id")
            if not eid:
                continue
            # Récupère les cotes pour cet event
            ro = requests.get(
                f"{ODDS_API_BASE}/odds",
                params={"apiKey": ODDS_API_KEY, "eventId": eid, "market": "1x2"},
                timeout=8
            )
            if ro.status_code != 200:
                continue
            odds_data = ro.json().get("data", {})
            bookmakers = odds_data.get("bookmakers", [])
            if not bookmakers:
                continue
            # Prend le premier bookmaker dispo
            bk = bookmakers[0]
            markets = bk.get("markets", [])
            h2h = next((m for m in markets if m.get("type") in ["1x2", "moneyline", "winner"]), None)
            if not h2h:
                continue
            outcomes = {o.get("name", ""): o.get("price") for o in h2h.get("outcomes", [])}
            key = f"{home}|||{away}"
            result[key] = {
                "1": outcomes.get(home) or outcomes.get("1") or outcomes.get("Home"),
                "X": outcomes.get("Draw") or outcomes.get("X"),
                "2": outcomes.get(away) or outcomes.get("2") or outcomes.get("Away"),
                "event_id_api": eid,
            }
        _odds_cache[sport] = result
        print(f"  ✅ Odds-API.io {sport}: {len(result)} matchs")
    except Exception as e:
        print(f"  ⚠️  Odds-API.io {sport}: {e}")
    return result

def fetch_value_bets(sport):
    """Récupère les value bets déjà calculés par odds-api.io."""
    if not ODDS_API_KEY or sport in _valuebets_cache:
        return _valuebets_cache.get(sport, [])
    sport_key = ODDS_SPORT_MAP.get(sport, sport)
    try:
        r = requests.get(
            f"{ODDS_API_BASE}/value-bets",
            params={"apiKey": ODDS_API_KEY, "sport": sport_key, "includeEventDetails": "true"},
            timeout=10
        )
        if r.status_code == 200:
            vbs = r.json().get("data", [])
            _valuebets_cache[sport] = vbs
            print(f"  ✅ Value bets {sport}: {len(vbs)} opportunités")
            return vbs
    except Exception as e:
        print(f"  ⚠️  Value bets {sport}: {e}")
    return []

def get_api_odds(home, away, sport):
    """Cherche les cotes d'un match dans le cache odds-api.io."""
    cache = fetch_odds_for_sport(sport)
    if not cache:
        return {}
...
def get_value_bet_for_match(home, away, sport):
    """Cherche si un match a un value bet détecté par odds-api.io."""
    vbs = fetch_value_bets(sport)
    for vb in vbs:
        ev = vb.get("event", {})
...
def get_best_odds(event_id, home, away, sport):
    """Odds-API.io en priorité, SofaScore en fallback."""
    api_odds = get_api_odds(home, away, sport)
    if api_odds.get("1") or api_odds.get("2"):
        return api_odds
    return get_odds(event_id)
...
WHITELIST_LEAGUES = {
    ...
    "basketball": [
        ...
        "ACB", "Liga ACB",
        "Lega Basket Serie A",
        "ABA Liga", "ABA Super League",
        "BBL", "Bundesliga Basketball",
        "Pro A", "Elite 2",
        "Stoiximan GBL",
        "NBL", "WNBL",
        ...
        "KBL",
        "VTB United League",
        "Turkish Airlines BSL", "BSL",
    ],
    "tennis": None,  # Tout accepté
    "ice-hockey": [
        "NHL", "AHL",
        "DEL",
        "National League",
    ...
    "rugby": [
        ...
        "Premiership Rugby", "English Premiership",
        "Super Rugby",
        "Six Nations", "Women's Six Nations",
        "Rugby World Cup",
    ],
}

def league_allowed(event, sport_key):
    """Vérifie si la ligue est dans la whitelist."""
    allowed = WHITELIST_LEAGUES.get(sport_key)
    if allowed is None:
        return True  # Tennis : tout accepté
    league_name = event.get("tournament", {}).get("name", "")
    unique_name = event.get("tournament", {}).get("uniqueTournament", {}).get("name", "")
    for w in allowed:
        if w.lower() in league_name.lower() or w.lower() in unique_name.lower():
            return True
    return False
...
def get_events(sport_key):
    ...
    try:
        ...
        # Filtrer par date ET par ligue
        filtered = [
            e for e in events
            if e.get("startTimestamp") and
            datetime.fromtimestamp(e["startTimestamp"]).date() == date.today() and
            league_allowed(e, sport_key)
        ]
...
def get_team_stats(team_id):
    """Récupère stats détaillées d'une équipe via SofaScore."""
    try:
        url  = f"https://api.sofascore.com/api/v1/team/{team_id}/unique-tournament-statistics/17"
        data = scraper.get(url, headers=headers, timeout=5).json()
        stats = data.get("statistics", {})
        return {
            "goals_scored":   stats.get("goalsScored", 0) or 0,
            "goals_conceded": stats.get("goalsConceded", 0) or 0,
            "matches":        stats.get("matches", 1) or 1,
            "wins":           stats.get("wins", 0) or 0,
            "draws":          stats.get("draws", 0) or 0,
            "losses":         stats.get("losses", 0) or 0,
            "home_goals":     stats.get("homeGoalsScored", 0) or 0,
            "away_goals":     stats.get("awayGoalsScored", 0) or 0,
            "home_matches":   stats.get("homeMatches", 1) or 1,
            "away_matches":   stats.get("awayMatches", 1) or 1,
            "shots_on_target":stats.get("shotsOnTarget", 0) or 0,
            "big_chances":    stats.get("bigChancesCreated", 0) or 0,
        }
    except:
        return {}

def get_standing(team_id):
    """Récupère la position au classement."""
    try:
        url  = f"https://api.sofascore.com/api/v1/team/{team_id}/standings"
        data = scraper.get(url, headers=headers, timeout=5).json()
...
def poisson_matrix(home_lam, away_lam, max_goals=6):
    """Génère la matrice des scores probables via Poisson."""
    import math
    ...
    for h in range(max_goals):
        for a in range(max_goals):
            ...
            top3[f"{h}-{a}"] = round(prob, 4)
    top3 = sorted(top3.items(), key=lambda x: x[1], reverse=True)[:5]
    return round(home_win, 3), round(draw, 3), round(away_win, 3), top3
...
def analyze_team_sport(sport_name, sport_key, slot=None):
    ...
    for event in events:
        ...
        league   = event.get("tournament", {}).get("name", sport_name)
        ts       = event.get("startTimestamp", 0)
        match_time = datetime.fromtimestamp(ts).strftime("%H:%M") if ts else "?"

        odds      = get_best_odds(event_id, home, away, sport_name)
        vbet      = get_value_bet_for_match(home, away, sport_name)
        ...
        # ── Stats avancées si foot ────────────────────────
        if sport_name == "football":
            home_stats = get_team_stats(home_id)
            away_stats = get_team_stats(away_id)
            home_std   = get_standing(home_id)
            away_std   = get_standing(away_id)

            # xG simplifié : buts marqués/match + shots
            home_matches = max(home_stats.get("matches", 1), 1)
            away_matches = max(away_stats.get("matches", 1), 1)
            home_gpg = home_stats.get("goals_scored", 0) / home_matches
            away_gpg = away_stats.get("goals_scored", 0) / away_matches
            home_cag = home_stats.get("goals_conceded", 0) / home_matches
            away_cag = away_stats.get("goals_conceded", 0) / away_matches

            # Lambda Poisson = attaque dom × défense ext (Dixon-Coles simplifié)
            league_avg = 1.35  # moyenne buts/équipe en championnat
            home_lam = round(max((home_gpg * away_cag / max(league_avg, 0.1)) * 1.1, 0.3), 2)  # +10% avantage dom
            away_lam = round(max((away_gpg * home_cag / max(league_avg, 0.1)), 0.3), 2)

            pred_home, pred_draw, pred_away, top5_scores = poisson_matrix(home_lam, away_lam)

            # Facteur classement
            home_pos = home_std.get("position", 10)
            away_pos = away_std.get("position", 10)
            if home_pos and away_pos:
                pos_factor = (away_pos - home_pos) / 20  # normalise
                pred_home = round(min(pred_home + pos_factor * 0.05, 0.85), 3)
                pred_away = round(max(pred_away - pos_factor * 0.05, 0.05), 3)
                pred_draw = round(max(1 - pred_home - pred_away, 0.05), 3)

            goals_exp  = round(home_lam + away_lam, 2)

            # Over/Under dynamique selon la ligue
            ou_line = 2.5
            over_under_25 = "⬆️ OVER 2.5" if goals_exp > 2.5 else "⬇️ UNDER 2.5"
            over_under_15 = "⬆️ OVER 1.5" if goals_exp > 1.5 else "⬇️ UNDER 1.5"
            ...
            extra_stats = {
                ...
                "home_cag": round(home_cag, 2),
                "away_cag": round(away_cag, 2),
                "home_lam": home_lam,
                "away_lam": away_lam,
                "btts_prob": btts_prob,
                "btts": btts,
                ...
                "home_pts": home_std.get("points", "?"),
                "away_pts": away_std.get("points", "?"),
            }
            w_h2h, w_form = 0.4, 0.6
        else:
            # Rugby / Hockey — algo standard
            pred_home, pred_draw, pred_away, w_h2h, w_form = predict(h2h, home_fs, away_fs)
            goals_exp   = round((0.5 + home_fs * 2.5) + (0.5 + away_fs * 2.5), 2)
            over_under_25 = "⬆️ OVER 2.5" if goals_exp > 2.5 else "⬇️ UNDER 2.5"
            top5_scores = predict_score(home_fs, away_fs)
            extra_stats = {}
        ...
        # Boost shadow score si value bet détecté par odds-api.io
        if vbet:
            score = min(score + 15, 100)

        if vbet:
            verdict = f"🎯 VALUE BET API: {vbet.get('selection','?')} @ {vbet.get('odd','?')} ({vbet.get('bookmaker','?')})"
...
def get_player_ranking(player_id):
    """Récupère le ranking ATP/WTA d'un joueur."""
    try:
        url  = f"https://api.sofascore.com/api/v1/team/{player_id}/statistics/season"
        data = scraper.get(url, headers=headers, timeout=5).json()
        return data.get("statistics", {}).get("ranking", 999)
    except:
        return 999
...
def analyze_tennis():
    ...
    for event in events:
        ...
        home_id  = event.get("homeTeam", {}).get("id")
        away_id  = event.get("awayTeam", {}).get("id")
        surface  = event.get("tournament", {}).get("groundType", "Hard") or "Hard"
        league   = event.get("tournament", {}).get("name", "Tennis")
        ts       = event.get("startTimestamp", 0)
        match_time = datetime.fromtimestamp(ts).strftime("%H:%M") if ts else "?"
        ...
        # ── Ranking → probabilité ─────────────────────────
        home_rank = get_player_ranking(home_id)
        away_rank = get_player_ranking(away_id)

        # Plus le ranking est bas (1 = meilleur), plus la proba est haute
        if home_rank < 999 and away_rank < 999:
            rank_total = home_rank + away_rank
            rank_prob_home = away_rank / rank_total
            rank_prob_away = home_rank / rank_total
        else:
            rank_prob_home = rank_prob_away = 0.5

        # ── Surface bonus ─────────────────────────────────
        # Avantage domicile (serveur) + surface
        surface_bonus = 0.03  # léger avantage joueur 1 (ordre de match)
        if "clay" in surface.lower() or "terre" in surface.lower():
            surface_label = "🧱 Terre battue"
        elif "grass" in surface.lower() or "gazon" in surface.lower():
            surface_label = "🌿 Gazon"
            surface_bonus = 0.04
        elif "indoor" in surface.lower():
            surface_label = "🏟️ Indoor"
        else:
            surface_label = "🪨 Dur"
        ...
        if h2h_total > 0:
            h2h_prob_home = h2h.get("home_wins", 0) / h2h_total
            h2h_prob_away = h2h.get("away_wins", 0) / h2h_total
        else:
            h2h_prob_home = h2h_prob_away = 0.5

        pred_home = round(
            rank_prob_home * w_rank +
            home_fs * w_form +
            h2h_prob_home * w_h2h +
            surface_bonus, 3
        )
        ...
        # Confiance basée sur ranking + H2H + forme
        confidence = 30
        if home_rank < 100 and away_rank < 100: confidence += 25
        elif home_rank < 200 and away_rank < 200: confidence += 15
        if h2h_total >= 3: confidence += 20
        ...
        results.append({
            ...
            "prediction": {
                ...
                "home_ranking": home_rank if home_rank < 999 else "?",
                "away_ranking": away_rank if away_rank < 999 else "?",
                "h2h_home":     h2h.get("home_wins", 0),
                "h2h_away":     h2h.get("away_wins", 0),
            },
            "value":        {"home": value_home, "away": value_away},
            "kelly":        {"home": kelly(pred_home, odd_1), "away": kelly(pred_away, odd_2)},
            "verdict":      verdict,
...
# ── ESPN : advanced stats (remplace Basketball-Reference) ──
_bbref_cache = {}

def get_bbref_advanced_stats():
    """Récupère OffRtg, DefRtg, Pace via ESPN — sans pandas."""
    if _bbref_cache:
        return _bbref_cache
    try:
        url  = "https://site.api.espn.com/apis/site/v2/sports/basketball/nba/teams"
        data = scraper.get(url, headers={"User-Agent": "Mozilla/5.0"}, timeout=10).json()
        ...
        for entry in teams:
            ...
            try:
                s = scraper.get(
                    f"https://site.api.espn.com/apis/site/v2/sports/basketball/nba/teams/{tid}/statistics",
                    headers={"User-Agent": "Mozilla/5.0"}, timeout=8
                ).json()
                cats = s.get("results", {}).get("stats", {}).get("categories", [])
                stats = {}
                for cat in cats:
                ...
                _bbref_cache[name] = {
                    ...
                    "pace":    float(stats.get("pace", 0)),
                    "efg_pct": float(stats.get("effectiveFieldGoalPct", 0)),
                    "net_rtg": float(stats.get("netRating", 0)),
                }
            except:
                continue
        print(f"  ✅ ESPN Advanced Stats: {len(_bbref_cache)} équipes")
    except Exception as e:
        print(f"  ⚠️  ESPN Advanced Stats: {e}")
    return _bbref_cache
...
def analyze_basketball(slot=None):
    ...
    for game in games:
        ...
        LEAGUE_OU = {
            ...
            "ACB": 168.5, "Liga ACB": 168.5,
            "Lega Basket": 162.5, "Serie A": 162.5,
        ...
        try:
            ...
            # Ligne O/U selon ligue
            ou_line = 224.5  # NBA par défaut
            for k, v in LEAGUE_OU.items():
                if k.lower() in "nba":
                    ou_line = v
                    break
    ...
    BASKET_OU = {
        "euroleague": 162.5, "eurocup": 158.5,
        "acb": 168.5, "liga acb": 168.5,
        "lega": 162.5, "serie a": 162.5,
        "bbl": 158.5, "pro a": 162.5, "elite": 162.5,
        "turkish": 165.5, "bsl": 165.5,
        "vtb": 158.5, "nbb": 162.5,
        "kbl": 158.5, "nbl": 168.5,
        "stoiximan": 162.5, "gbl": 162.5,
        "champions": 160.5,
        "ncaa": 145.5, "wnba": 168.5,
    }
    ...
    for event in events:
        ...
        home_id    = event.get("homeTeam", {}).get("id")
        away_id    = event.get("awayTeam", {}).get("id")
        league     = event.get("tournament", {}).get("name", "Other")
        ts         = event.get("startTimestamp", 0)
        match_time = datetime.fromtimestamp(ts).strftime("%H:%M") if ts else "?"
        ...
        # O/U dynamique selon la ligue
        ou_line = 162.5  # défaut ligues européennes
        league_lower = league.lower()
        for k, v in BASKET_OU.items():
            if k in league_lower:
                ou_line = v
                break
        total_pts  = round((home_fs * ou_line * 0.9) + (away_fs * ou_line * 0.9), 1)
        if total_pts == 0: total_pts = ou_line
        over_under = f"⬆️ OVER {ou_line} ({total_pts}pts)" if total_pts > ou_line else f"⬇️ UNDER {ou_line} ({total_pts}pts)"
