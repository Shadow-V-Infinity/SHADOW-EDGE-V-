...
def form_score(form: list) -> float:
    """Calcule un score de forme basé sur les résultats récents (W/D/L)."""
    if not form:
        return 0.5
    return form.count("W") / len(form)

def get_trend(form: list) -> str:
    """Retourne une tendance textuelle basée sur le taux de victoire."""
    if not form:
        return "➖ AUCUNE DONNÉE"
    win_rate = form.count("W") / len(form)
    if   win_rate >= 0.7: return "📈 EXCELLENTE"
    elif win_rate >= 0.5: return "📊 BONNE"
    elif win_rate >= 0.3: return "➖ MOYENNE"
    else:                 return "📉 MAUVAISE"

def kelly(prob: float, odd: float) -> float:
    """Calcule le critère de Kelly pour une probabilité et une cote données."""
    if not odd or odd <= 1:
        return 0
    return round((prob * odd - (1 - prob)) / odd, 3)

def shadow_score(confidence: int, best_value: float, best_kelly: float,
                 has_lineup: bool = False, w_impact: int = 0) -> int:
    """Calcule un score global basé sur la confiance, la value et le critère de Kelly."""
    value_contribution = min(best_value * 20, 30) if best_value else 0
    kelly_contribution = min(best_kelly * 10, 30) if best_kelly else 0
    s = confidence + value_contribution + kelly_contribution
    if has_lineup: s += 10
    s += w_impact
    return min(round(s), 100)

def pick_label(score: int) -> str:
    """Retourne une étiquette qualitative basée sur le score."""
    if   score >= 90: return "⭐⭐⭐ EXCELLENT"
    elif score >= 75: return "⭐⭐ TRÈS BON"
    elif score >= 50: return "⭐ BON"
    else:             return "⚪ ORDINAIRE"

# ════════════════════════════════════════════════════════
# POISSON (FOOT) — version enrichie
# ════════════════════════════════════════════════════════

from scipy.stats import poisson as scipy_poisson
import numpy as np

def poisson_over_under(lam, threshold=0.5, is_half_time=False):
    """Calcule les probabilités Over/Under pour un seuil donné."""
    if is_half_time:
        lam = lam / 2
    k = int(threshold)
    under_prob = scipy_poisson.cdf(k, lam)
    over_prob = 1 - under_prob
    return round(over_prob, 3), round(under_prob, 3)

def poisson_matrix(home_lam, away_lam, max_goals=6, return_matrix=False, over_under_thresholds=[0.5, 1.5, 2.5]):
    """Calcule les probabilités de victoire/nul/défaite, les scores, et Over/Under."""
    max_goals = max(max_goals, int(home_lam + away_lam + 3 * max(home_lam, away_lam)**0.5))
    ...
    for h in range(max_goals):
        for a in range(max_goals):
            prob = scipy_poisson.pmf(h, home_lam) * scipy_poisson.pmf(a, away_lam)
    ...
    over_under_full = {th: poisson_over_under(total_lam, th) for th in over_under_thresholds}
    over_under_half = {th: poisson_over_under(total_lam, th, is_half_time=True) for th in over_under_thresholds}

    if return_matrix:
        return round(home_win, 3), round(draw, 3), round(away_win, 3), top5, prob_matrix, over_under_full, over_under_half
    else:
        return round(home_win, 3), round(draw, 3), round(away_win, 3), top5, over_under_full, over_under_half

def poisson_with_handicap(home_lam, away_lam, handicap=0):
    """Calcule les probabilités avec un handicap."""
    home_win = draw = away_win = 0
    for h in range(20):
        for a in range(20):
            prob = scipy_poisson.pmf(h, home_lam) * scipy_poisson.pmf(a, away_lam)
            if h > a + handicap:   home_win += prob
            elif h == a + handicap: draw += prob
            else:                   away_win += prob
    return home_win, draw, away_win

def adjust_lambda_for_form(home_lam, away_lam, home_form, away_form, weight=0.5):
    """Ajuste les lambdas en fonction de la forme récente."""
    adjusted_home_lam = home_lam * (1 - weight) + home_form * weight
    adjusted_away_lam = away_lam * (1 - weight) + away_form * weight
    return adjusted_home_lam, adjusted_away_lam

def double_chance_probabilities(home_win, draw, away_win):
    """Calcule les probabilités de double chance."""
    return {
...
def get_poisson_prob(home_lam, away_lam, market_odds=None, handicap=0, home_form=None, away_form=None):
    """Calcule toutes les probabilités et cotes."""
    if home_form is not None and away_form is not None:
        home_lam, away_lam = adjust_lambda_for_form(home_lam, away_lam, home_form, away_form)

    home_win, draw, away_win, top5, over_under_full, over_under_half = poisson_matrix(home_lam, away_lam)
    home_win_hc, draw_hc, away_win_hc = poisson_with_handicap(home_lam, away_lam, handicap)

    home_odds_fair = round(1 / home_win, 2) if home_win else None
    draw_odds_fair = round(1 / draw, 2)     if draw     else None
    away_odds_fair = round(1 / away_win, 2) if away_win else None

    value = {}
    if market_odds:
        def calc_value(fair, market):
            return round((fair - market) / market * 100, 2) if fair and market else None
        value = {
            "home_value": calc_value(home_odds_fair, market_odds.get("home")),
            "draw_value": calc_value(draw_odds_fair, market_odds.get("draw")),
            "away_value": calc_value(away_odds_fair, market_odds.get("away")),
        }

    double_chance = double_chance_probabilities(home_win, draw, away_win)

    return {
        "home_win": home_win, "draw": draw, "away_win": away_win,
        "top5_scores": top5,
        "over_under_full": over_under_full,
        "over_under_half": over_under_half,
        "home_odds": home_odds_fair,
        "draw_odds": draw_odds_fair,
        "away_odds": away_odds_fair,
        "value": value,
        "double_chance": double_chance,
        "home_win_hc": home_win_hc,
        "draw_hc": draw_hc,
        "away_win_hc": away_win_hc,
    }
    ...
    for m in matches:
        ...
        try:
            ...
            if probas.get("home_win"):
                ...
                _, _, _, top5_scores, ou_full, ou_half = poisson_matrix(home_lam, away_lam)
                goals_exp = round(home_lam + away_lam, 2)
            else:
                # Fallback Poisson
                home_lam = round(max((home_fs * 2.5 + 0.5), 0.3), 2) * 1.1
                away_lam = round(max((away_fs * 2.5 + 0.5), 0.3), 2)
                pred_home, pred_draw, pred_away, top5_scores, ou_full, ou_half = poisson_matrix(home_lam, away_lam)
                goals_exp = round(home_lam + away_lam, 2)
                btts_prob = round((1 - math.exp(-home_lam)) * (1 - math.exp(-away_lam)), 3)
                over_25 = ou_full.get(2.5, (0.5, 0.5))[0]
                over_15 = ou_full.get(1.5, (0.7, 0.3))[0]
            ...
            results.append({
                ...
                "prediction": {
                    ...
                    "home_lam":       home_lam,
                    "away_lam":       away_lam,
                    "over_under_full": ou_full,
                    "over_under_half": ou_half,
                    "double_chance":   double_chance_probabilities(pred_home, pred_draw, pred_away),
                    "home_win_hc":     round(poisson_with_handicap(home_lam, away_lam, 0)[0], 3),
                },
...
def get_halftime_stats_hockey(team_id: str, sport_key: str) -> dict:
    """Statistiques après 2 périodes (40 min) pour le hockey."""
    # À implémenter : récupération via API ou BDD
    return {"avg_goals_after_2p": 1.5, "last_5_halftime_goals": [1, 0, 1, 2, 1]}

def get_halftime_stats_rugby(team_id: str, sport_key: str) -> dict:
    """Statistiques en 1ère mi-temps (40 min) pour le rugby."""
    # À implémenter : récupération via API ou BDD
    return {"avg_points_1h": 10.0, "last_5_halftime_scores": [7, 10, 3, 14, 10]}

def get_halftime_odds_hockey(match_id: str, sport_key: str) -> dict:
    """Cotes Over/Under après 2 périodes pour le hockey."""
    # À implémenter : récupération via API
    return {"over_3.5": 1.9, "under_3.5": 1.85}

def get_halftime_odds_rugby(match_id: str, sport_key: str) -> dict:
    """Cotes Over/Under à la mi-temps pour le rugby."""
    # À implémenter : récupération via API
    return {"over_12.5": 1.8, "under_12.5": 1.95}

def analyze_team_sport(sport_name: str, sport_key: str, leagues: dict):
    print(f"\n🏒🏉 Analyse {sport_name}...")
    ...
    for m in matches:
        ...
        try:
            ...
            # ── Prédiction H2H + forme (pondération dynamique) ──
            t = h2h.get("total", 0)
            ...
            pred_home = h2h_ph * wh + home_fs * wf
            pred_away = h2h_pa * wh + away_fs * wf
            pred_draw = 1 - pred_home - pred_away
            ...
            # ── Analyse Over/Under mi-match ──────────────────────
            halftime_analysis = None
            if sport_key == "hockey":
                ht_odds  = get_halftime_odds_hockey(match_id, sport_key)
                home_hts = get_halftime_stats_hockey(home_id, sport_key)
                away_hts = get_halftime_stats_hockey(away_id, sport_key)
                threshold = 3.5
                avg_key   = "avg_goals_after_2p"
            elif sport_key == "rugby":
                ht_odds  = get_halftime_odds_rugby(match_id, sport_key)
                home_hts = get_halftime_stats_rugby(home_id, sport_key)
                away_hts = get_halftime_stats_rugby(away_id, sport_key)
                threshold = 12.5
                avg_key   = "avg_points_1h"

            if sport_key in ("hockey", "rugby"):
                avg_home  = home_hts.get(avg_key, 0)
                avg_away  = away_hts.get(avg_key, 0)
                avg_total = avg_home + avg_away

                pred_over  = min(avg_total / threshold, 1.0)
                pred_under = round(1 - pred_over, 3)
                pred_over  = round(pred_over, 3)

                odd_over   = ht_odds.get(f"over_{threshold}")
                odd_under  = ht_odds.get(f"under_{threshold}")
                ip_over    = round(1 / odd_over,  3) if odd_over  else None
                ip_under   = round(1 / odd_under, 3) if odd_under else None
                value_over  = round(pred_over  - ip_over,  3) if ip_over  else None
                value_under = round(pred_under - ip_under, 3) if ip_under else None

                ht_verdict = (
                    f"🔥 VALUE OVER {threshold}"  if value_over  and value_over  > 0.05 else
                    f"🔥 VALUE UNDER {threshold}" if value_under and value_under > 0.05 else
                    "⚪ Pas de value"
                )
                ...
                halftime_analysis = {
                    ...
                    "value":       {"over": value_over, "under": value_under},
                    "verdict":     ht_verdict,
                }
            ...
            if halftime_analysis:
                print(f"     Over/Under {threshold} mi-match: {halftime_analysis['verdict']}")
